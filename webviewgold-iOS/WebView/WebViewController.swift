...
class WebViewController: UIViewController, OSSubscriptionObserver, GADBannerViewDelegate, SKStoreProductViewControllerDelegate,SKProductsRequestDelegate, SKPaymentTransactionObserver, FBInterstitialAdDelegate, GADFullScreenContentDelegate
{
    ...
    /// Early splash dismiss (see `splashDismissAtEstimatedProgress` in Config)
    private var splashProgressObservation: NSKeyValueObservation?
    private var initialSplashDismissed = false

    /// Reveal WebView as soon as navigation starts or progress threshold hits (don’t wait for full load).
    private func hideInitialSplashOverlay() {
        guard splashScreenEnabled, !initialSplashDismissed else { return }
    ...
    deinit {
        ...
        splashProgressObservation?.invalidate()
    ...
    override func viewDidLoad()
    {
        ...
        if (splashScreenEnabled) {
            ...
            if scaleSplashImage != 100 { // Non-Fullscreen Mode — width for centered splash
        ...
        // Defer StoreKit so network timeouts (DownloadFailed) don’t compete with first WebView load
        ...
        splashProgressObservation?.invalidate()
        if splashDismissAtEstimatedProgress > 0 && splashDismissAtEstimatedProgress < 1.0
            && splashScreenEnabled && remainSplashOption {
            splashProgressObservation = webView.observe(\.estimatedProgress, options: [.new]) { [weak self] wv, _ in
                guard let self = self else { return }
                guard !self.initialSplashDismissed else { return }
                if wv.estimatedProgress >= splashDismissAtEstimatedProgress {
                    self.hideInitialSplashOverlay()
                }
            }
        }
        ...
        tuneWebViewScrolling(webView, isMainWebView: true)
...
extension WebViewController
{
    /// Stops horizontal rubber-banding (blank edges when swiping sideways). Vertical bounce still allowed when using pull-to-refresh.
    private func tuneWebViewScrolling(_ webView: WKWebView, isMainWebView: Bool) {
        webView.scrollView.alwaysBounceHorizontal = false
        webView.scrollView.isDirectionalLockEnabled = true
        if preventoverscroll {
            webView.scrollView.bounces = false
        } else if isMainWebView && pulltorefresh {
...
extension WebViewController: WKUIDelegate
{
    func webView(_ webView: WKWebView, createWebViewWith configuration: WKWebViewConfiguration, for navigationAction: WKNavigationAction, windowFeatures: WKWindowFeatures) -> WKWebView? {
        ...
        if (google_login_helper_enabled || facebook_login_helper_enabled) {
            ...
            tuneWebViewScrolling(loginPopupWindow!, isMainWebView: false)
