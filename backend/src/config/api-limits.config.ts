 ...
 * Override via env vars when upgrading plan or tuning for your quota.
 *
 * Supported plans:
 * - free
 * - pro
...
export const API_SPORTS_PLAN: 'free' | 'pro' | 'ultra' = (() => {
  const plan = (process.env.API_SPORTS_PLAN || 'free').toLowerCase();
  if (plan === 'ultra') return 'ultra';
  if (plan === 'pro') return 'pro';
  return 'free';
})();

const isProLike = API_SPORTS_PLAN === 'pro' || API_SPORTS_PLAN === 'ultra';
const isUltra = API_SPORTS_PLAN === 'ultra';

/** Lookahead days for fixture/game sync (today through today+N) */
export const SYNC_LOOKAHEAD_DAYS = Math.min(
  Math.max(parseInt(process.env.API_SYNC_LOOKAHEAD_DAYS || '', 10) || 3, 1),
  14
);

/** Max events without odds to fetch odds for per sync run (0 = uncapped). */
export const MAX_ODDS_EVENTS_PER_RUN = Math.max(
  parseInt(process.env.API_MAX_ODDS_EVENTS_PER_RUN || '', 10) || (isUltra ? 0 : isProLike ? 300 : 50),
  0
);

/** Max football fixtures without odds per sync run (0 = uncapped). */
export const MAX_FOOTBALL_ODDS_FIXTURES = Math.max(
  parseInt(process.env.API_MAX_FOOTBALL_ODDS_FIXTURES || '', 10) || (isUltra ? 0 : isProLike ? 300 : 100),
  0
);

/** Max leagues to backfill metadata per sync (league details API) */
export const MAX_LEAGUE_BACKFILL_PER_RUN = Math.min(
  Math.max(parseInt(process.env.API_MAX_LEAGUE_BACKFILL_PER_RUN || '', 10) || (isUltra ? 100 : isProLike ? 50 : 30), 1),
  100
);
...
export const MAX_FIXTURES_TO_UPDATE_PER_RUN = Math.min(
  Math.max(parseInt(process.env.API_MAX_FIXTURES_UPDATE_PER_RUN || '', 10) || (isUltra ? 200 : isProLike ? 100 : 50), 1),
  200
);

/** Delay between prediction API calls (ms) */
export const PREDICTION_API_DELAY_MS = Math.max(
  parseInt(process.env.API_PREDICTION_DELAY_MS || '', 10) || (isUltra ? 50 : isProLike ? 100 : 150),
  50
);
