...
@Injectable()
export class RugbySyncService {
  private readonly logger = new Logger(RugbySyncService.name);
  ...
  async sync(): Promise<{ games: number; odds: number }> {
    if (!isSportEnabled('rugby')) return { games: 0, odds: 0 };

    const sportKeys = await this.oddsApi.getActiveSportKeys(SPORT_ODDS_PREFIXES.rugby);
    if (!sportKeys.length) {
      this.logger.log('Rugby sync: no active competitions');
    ...
    for (const sportKey of sportKeys) {
      ...
      for (const evt of events) {
        ...
        const saved = await this.sportEventRepo.findOne({ where: { sport: 'rugby', apiId } });
        if (saved) {
    ...
    this.logger.log(`Rugby sync: ${gamesCount} matches, ${oddsCount} with odds (${sportKeys.length} competitions)`);
    return { games: gamesCount, odds: oddsCount };
  }

  @Cron('20 0 * * *') // 12:20 AM — consolidated to midnight window
