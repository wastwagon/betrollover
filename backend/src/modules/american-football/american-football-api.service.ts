...
export interface ApiAmericanFootballGame {
  id: number;
  date: string;
...
@Injectable()
export class AmericanFootballApiService {
  constructor(
  ...
  async getGames(date: string): Promise<ApiAmericanFootballGame[]> {
    const key = await this.getApiKey();
    if (!key) return [];
    const base = getSportApiBaseUrl('american_football');
    const res = await fetch(`${base}/games?date=${date}`, {
      headers: { 'x-apisports-key': key },
    });
    const data = await safeJson<any>(res);
    if (data?.errors && Object.keys(data.errors).length > 0) return [];
    return (data?.response || []) as ApiAmericanFootballGame[];
  }
