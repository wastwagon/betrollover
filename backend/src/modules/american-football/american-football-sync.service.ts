...
@Injectable()
export class AmericanFootballSyncService {
  private readonly logger = new Logger(AmericanFootballSyncService.name);
  ...
  async sync(): Promise<{ games: number; odds: number }> {
    if (!isSportEnabled('american_football')) return { games: 0, odds: 0 };

    const sportKeys = await this.oddsApi.getActiveSportKeys(SPORT_ODDS_PREFIXES.american_football);
    if (!sportKeys.length) {
      this.logger.log('American Football sync: no active competitions');
    ...
    for (const sportKey of sportKeys) {
      ...
      for (const evt of events) {
        ...
        const saved = await this.sportEventRepo.findOne({
          where: { sport: 'american_football', apiId },
        });
        if (saved) {
    ...
    this.logger.log(
      `American Football sync: ${gamesCount} games, ${oddsCount} with odds (${sportKeys.length} competitions)`,
    );
    return { games: gamesCount, odds: oddsCount };
  }

  @Cron('45 0 * * *') // 12:45 AM — consolidated to midnight window
