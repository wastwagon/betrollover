...
@Injectable()
export class TennisSyncService {
  ...
  async sync(): Promise<{ games: number; odds: number }> {
    if (!isSportEnabled('tennis')) return { games: 0, odds: 0 };

    const sportKeys = await this.oddsApi.getActiveSportKeys(SPORT_ODDS_PREFIXES.tennis);
    if (!sportKeys.length) {
      this.logger.log('Tennis sync: no active tournaments');
    ...
    for (const sportKey of sportKeys) {
      ...
      for (const evt of events) {
        ...
        const saved = await this.sportEventRepo.findOne({ where: { sport: 'tennis', apiId } });
        if (saved) {
    ...
    this.logger.log(`Tennis sync: ${gamesCount} matches, ${oddsCount} with odds (${sportKeys.length} tournaments)`);
    return { games: gamesCount, odds: oddsCount };
  }

  @Cron('50 0 * * *') // 12:50 AM — consolidated to midnight window
