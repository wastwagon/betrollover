...
export class TennisController {
  ...
  async getEvents(@Query('days') days?: string) {
    if (!isSportEnabled('tennis')) return { events: [] };
    const now = new Date();
    ...
    const events = await this.sportEventRepo.find({
      where: { sport: 'tennis', status: 'NS', eventDate: MoreThanOrEqual(now) },
      relations: ['odds'],
      order: { eventDate: 'ASC' },
      take: 200,
    });
    ...
    return {
      events: events.filter((e) => e.eventDate <= end && (e.odds?.length ?? 0) > 0),
    };
  }
}
