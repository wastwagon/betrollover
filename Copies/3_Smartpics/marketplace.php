...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'purchase_pick') {
    ...
    if ($pickId) {
        try {
            ...
            header('Location: /SmartPicksPro-Local/marketplace');
            exit;
    ...
    <div class="card">
        ...
        <form method="GET" style="display: flex; gap: 15px; align-items: center; flex-wrap: wrap;">
            ...
            <div style="display: flex; gap: 10px; align-items: end;">
                ...
                <a href="/SmartPicksPro-Local/marketplace" class="btn btn-secondary" style="padding: 8px 16px;">
...
function searchTipsters(query) {
    ...
    searchTimeout = setTimeout(() => {
        fetch(`/SmartPicksPro-Local/api/search_tipsters.php?q=${encodeURIComponent(query)}`)
...
function topUpWallet() {
    window.location.href = '/SmartPicksPro-Local/wallet';
}

function viewCoupon(accumulatorId) {
    // Increment view count
    fetch(`/SmartPicksPro-Local/api/increment_view_count.php?accumulator_id=${accumulatorId}`, {
        method: 'POST'
    ...
    fetch(`/SmartPicksPro-Local/api/get_coupon_details.php?accumulator_id=${accumulatorId}`)
        .then(response => response.json())
        .then(data => {
            if (data.success) {
                ...
                let content = `
                    <div style="text-align: left;">
                        <div style="background: linear-gradient(135deg, #d32f2f, #f44336); color: white; padding: 20px; border-radius: 8px; margin-bottom: 20px;">
                            <h4 style="margin: 0 0 10px 0;">${coupon.title}</h4>
                            <p style="margin: 0; opacity: 0.9;">${coupon.description}</p>
                        </div>
                        
                        <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 15px; margin-bottom: 20px;">
                            <div style="background: #f8f9fa; padding: 15px; border-radius: 8px;">
                                <strong>Total Odds:</strong><br>
                                <span style="color: #2e7d32; font-size: 18px; font-weight: bold;">${parseFloat(coupon.total_odds).toFixed(2)}</span>
                            </div>
                            <div style="background: #f8f9fa; padding: 15px; border-radius: 8px;">
                            ...
                            <div style="background: #f8f9fa; padding: 15px; border-radius: 8px;">
                                ...
                                <span style="color: ${coupon.price == 0 ? '#17a2b8' : '#2e7d32'}; font-size: 18px; font-weight: bold;">
                                    ${coupon.price == 0 ? 'FREE' : 'GHS ' + parseFloat(coupon.price).toFixed(2)}
                                </span>
                            </div>
                        </div>
                        ...
                        <div style="border: 1px solid #e0e0e0; border-radius: 8px; padding: 15px; margin-bottom: 10px;">
                            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 10px;">
                                ...
                                <span style="background: #2e7d32; color: white; padding: 4px 8px; border-radius: 4px; font-size: 12px;">
                                    ${parseFloat(pick.odds).toFixed(2)}
                                </span>
                            </div>
                            <div style="margin-bottom: 8px;">
                                <strong>Match:</strong> ${pick.match_description}
                            </div>
                            <div style="margin-bottom: 8px;">
                                <strong>Prediction:</strong> ${pick.prediction}
                            </div>
                            <div style="margin-bottom: 8px;">
                                <strong>Match Time:</strong> ${pick.match_date ? new Date(pick.match_date).toLocaleString() : 'Not specified'}
                            </div>
                            <div>
                                <strong>Status:</strong> 
                                <span style="color: ${pick.result === 'pending' ? '#ff9800' : pick.result === 'won' ? '#2e7d32' : '#d32f2f'}; font-weight: bold;">
        ...
        .catch(error => {
            document.getElementById('coupon-content').innerHTML = `
                <div style="color: #d32f2f;">
                    <i class="fas fa-exclamation-triangle" style="font-size: 24px; margin-bottom: 10px;"></i>
                    <p>Error loading coupon details. Please try again.</p>
                </div>
            `;
        });
}
