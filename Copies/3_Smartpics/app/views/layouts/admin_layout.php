...
<head>
    ...
    <style>
        * {
        ...
        /* Ensure notifications page uses full width by compensating for content-area padding */
        .content-area .notifications-page {
        ...
        .notifications-page .notification-item > div {
            width: 100% !important;
            box-sizing: border-box !important;
        }
        ...
        /* Ensure checkboxes in flex labels display correctly */
        label[style*="display: flex"] input[type="checkbox"],
        label[style*="display:flex"] input[type="checkbox"] {
            flex-shrink: 0;
        }
        
        /* Checkbox label styling - support for :has() and fallback */
        ...
        /* For browsers that don't support :has() - labels with inline flex styles already work */
...
<body>
    ...
    <script>
        // Detect base URL - only use /SmartPicksPro-Local for localhost
        const isLocalhost = window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1';
        const baseUrl = (isLocalhost && window.location.pathname.includes('/SmartPicksPro-Local')) ? '/SmartPicksPro-Local' : '';
            ...
            fetch(baseUrl + '/api/get_notifications.php?limit=10')
                ...
                .then(data => {
                    if (data.success) {
                        ...
                        list.innerHTML = '<div class="notification-empty"><i class="fas fa-exclamation-circle"></i>' + (data.error || 'Failed to load notifications') + '</div>';
                    }
                })
                .catch(error => {
                    console.error('Error loading notifications:', error);
                    list.innerHTML = '<div class="notification-empty"><i class="fas fa-exclamation-triangle"></i>Error loading notifications. Please try again.</div>';
                });
        }
        ...
        function displayNotifications(notifications) {
            ...
            if (!notifications || notifications.length === 0) {
                list.innerHTML = '<div class="notification-empty"><i class="fas fa-bell-slash"></i>No notifications</div>';
                return;
            }
            ...
            list.innerHTML = notifications.map(n => {
                const iconColors = {
                    ...
                    'pick_lost': '#d32f2f',
                    'pick_created': '#2e7d32',
                    'pick_pending_approval': '#ff9800'
                };
                const iconColor = iconColors[n.type] || '#1976d2';
                const iconClass = getIconClass(n.icon || 'bell');
                ...
                return `
                    <div class="notification-item ${unreadClass}" onclick="handleNotificationClick(${notificationId}, '${safeLink}')">
                        ...
                        <div class="notification-content">
                            ...
                            <div class="notification-time">${escapeHtml(n.time_ago || 'Just now')}</div>
                        </div>
                    </div>
                `;
            }).join('');
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            document.addEventListener('click', function(event) {
                ...
                if (dropdown && container) {
                    // Don't close if clicking inside the container (bell or dropdown)
