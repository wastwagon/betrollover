        ...
        if ($pickId && in_array($decision, ['approve', 'reject'])) {
            try {
                ...
                try {
                    ...
                    $pickInfo = $db->fetch("
                        SELECT at.title, at.user_id, u.username as tipster_name, u.display_name as tipster_display_name
                        FROM accumulator_tickets at
                        JOIN users u ON at.user_id = u.id
                        WHERE at.id = ?
                    ", [$pickId]);
                    ...
                    if ($pickInfo) {
                        ...
                        $tipsterName = $pickInfo['tipster_display_name'] ?? $pickInfo['tipster_name'] ?? 'a tipster';
                        ...
                        if ($decision === 'approve') {
                            ...
                            // Send in-app notification to tipster
                            ...
                            // Send in-app notification to admin who approved
                            try {
                                $notifyResult = $notificationService->notify(
                                    $userId,
                                    'admin_action',
                                    'Pick Approved',
                                    "You have successfully approved the pick '{$pickTitle}' from {$tipsterName}.",
                                    '/admin_approve_pick',
                                    [
                                        'pick_id' => $pickId,
                            ...
                            // Send in-app notification to admin who rejected
                            try {
                                $notifyResult = $notificationService->notify(
                                    $userId,
                                    'admin_action',
                                    'Pick Rejected',
                                    "You have rejected the pick '{$pickTitle}' from {$tipsterName}.",
                                    '/admin_approve_pick',
                                    [
                                        'pick_id' => $pickId,
                                        'tipster_id' => $tipsterId,
...
function viewTipsterProfile(tipsterName) {
    ...
    <style>
        ...
        /* Ensure page header doesn't block notifications */
        .page-header {
            position: relative;
    ...
    <script>
        ...
        // Just use the existing one or set it if it doesn't exist
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Ensure notification container is clickable
        ...
        function viewPick(pickId) {
            ...
            fetch(`${baseUrl}/api/get_pick_details.php?pick_id=${pickId}`)
        ...
        function editPick(pickId) {
            window.location.href = `${baseUrl}/edit_pick?id=${pickId}`;
        }
