<?php
/**
 * SmartPicks Pro - Self-Contained Admin All Picks
 */
...
try {
    ...
    if ($search) {
        $whereClause = "WHERE (at.title LIKE ? OR at.description LIKE ? OR u.username LIKE ?)";
        $searchTerm = '%' . $search . '%';
    ...
    switch ($filter) {
        case 'pending':
            $whereClause .= ($whereClause ? ' AND ' : 'WHERE ') . "at.status = 'pending'";
            break;
        case 'active':
            $whereClause .= ($whereClause ? ' AND ' : 'WHERE ') . "at.status = 'active'";
            break;
        case 'rejected':
            $whereClause .= ($whereClause ? ' AND ' : 'WHERE ') . "at.status = 'rejected'";
            break;
        case 'settled':
            $whereClause .= ($whereClause ? ' AND ' : 'WHERE ') . "at.status = 'settled'";
            break;
    }
    ...
    $error = 'Error loading picks data: ' . $e->getMessage();
    $logger->error('Picks data loading failed', ['error' => $e->getMessage()]);
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'update_status' && $pickId) {
        $newStatus = $_POST['status'] ?? '';
        try {
            $result = $db->query("UPDATE accumulator_tickets SET status = ?, updated_at = NOW() WHERE id = ?", [$newStatus, $pickId]);
            if ($result) {
                $success = 'Pick status updated successfully!';
                // Refresh the list
                $allPicks = $db->fetchAll("
                    SELECT 
                        at.id,
...
<head>
    ...
    <title>All Picks - SmartPicks Pro</title>
...
<body>
    <div class="sidebar" id="sidebar">
        ...
        <ul class="sidebar-menu">
            ...
            <li><a href="/admin_picks" class="active"><i class="fas fa-chart-line"></i> All Picks</a></li>
    ...
    <div class="main-content-wrapper">
        <div class="top-bar">
            <div class="page-title-section">
                <div class="page-title">All Picks</div>
        ...
        <div class="content-area">
            ...
            <div class="kpi-cards">
                ...
                <div class="kpi-card">
                    ...
                    <div class="card-value">
                        <?= count(array_filter($allPicks, function($pick) { return $pick['status'] === 'pending'; })) ?>
                ...
                <div class="kpi-card">
                    ...
                    <div class="card-value">
                        <?= count(array_filter($allPicks, function($pick) { return $pick['status'] === 'active'; })) ?>
                ...
                <div class="kpi-card">
                    ...
                    <div class="card-value">
                        GHS <?= number_format(array_sum(array_column($allPicks, 'price')), 2) ?>
            ...
            <div class="table-container">
                <h3><i class="fas fa-chart-line"></i> All Picks Management</h3>
                        ...
                        <tbody>
                            <?php foreach ($allPicks as $pick): ?>
                                <tr>
                                    ...
                                    <td><?= htmlspecialchars($pick['display_name'] ?? $pick['username']) ?></td>
                                    <td><?= htmlspecialchars($pick['sport']) ?></td>
                                    <td style="font-weight: bold; color: #D32F2F;"><?= number_format($pick['total_odds'], 2) ?></td>
                                    ...
                                    <td>
                                        <div style="display: flex; gap: 5px;">
                                            ...
                                            <button class="btn btn-primary" onclick="updateStatus(<?= $pick['id'] ?>, '<?= $pick['status'] ?>')">
                ...
                <?php else: ?>
                    <div style="text-align: center; padding: 40px; color: #666;">
                        <i class="fas fa-chart-line" style="font-size: 48px; color: #dc3545; margin-bottom: 15px;"></i>
            ...
            <div style="background: #e8f5e9; padding: 15px; border-radius: 8px; color: #2e7d32; margin-top: 30px;">
                <h4>📊 All Picks Management</h4>
                <p>Monitor and manage all picks across the platform. Update statuses and track performance.</p>
                <p><strong>Current Status:</strong> <?= count($allPicks) ?> total picks | <?= count(array_filter($allPicks, function($pick) { return $pick['status'] === 'active'; })) ?> active | <?= count(array_filter($allPicks, function($pick) { return $pick['status'] === 'pending'; })) ?> pending</p>
            </div>
        </div>
    </div>
    ...
    <div id="statusModal" style="display: none; position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0,0,0,0.5); z-index: 2000;">
        <div style="position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%); background: white; padding: 30px; border-radius: 8px; min-width: 400px;">
            <h3>Update Pick Status</h3>
            <form method="post" id="statusForm">
                <input type="hidden" name="action" value="update_status">
                <input type="hidden" name="pick_id" id="statusPickId">
    ...
    <script>
        ...
        function updateStatus(pickId, currentStatus) {
            document.getElementById('statusPickId').value = pickId;
