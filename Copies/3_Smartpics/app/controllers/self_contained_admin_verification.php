<?php
/**
 * SmartPicks Pro - Self-Contained Admin Verification
 */
...
try {
    ...
    $verificationApplications = $verificationSystem->getVerificationApplications();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'approve_verification' && $applicationId) {
        try {
            $result = $db->query("UPDATE tipster_verification_applications SET status = 'approved', reviewed_at = NOW() WHERE id = ?", [$applicationId]);
            if ($result) {
                $success = 'Verification application approved successfully!';
                // Refresh the list
                $verificationApplications = $verificationSystem->getVerificationApplications();
            } else {
        ...
        try {
            $result = $db->query("UPDATE tipster_verification_applications SET status = 'rejected', rejection_reason = ?, reviewed_at = NOW() WHERE id = ?", [$rejectionReason, $applicationId]);
            if ($result) {
...
<head>
    ...
    <title>Verification Management - SmartPicks Pro</title>
...
<body>
    <div class="sidebar" id="sidebar">
        ...
        <ul class="sidebar-menu">
            ...
            <li><a href="/admin_verification" class="active"><i class="fas fa-check-circle"></i> Verification</a></li>
    ...
    <div class="main-content-wrapper">
        <div class="top-bar">
            <div class="page-title-section">
                <div class="page-title">Verification Management</div>
        ...
        <div class="content-area">
            ...
            <div class="kpi-cards">
                <div class="kpi-card">
                    ...
                    <div class="card-value">
                        <?= count(array_filter($verificationApplications, function($app) { return $app['status'] === 'pending'; })) ?>
                ...
                <div class="kpi-card">
                    ...
                    <div class="card-value">
                        <?= count(array_filter($verificationApplications, function($app) { return $app['status'] === 'approved' && date('Y-m-d', strtotime($app['reviewed_at'] ?? '')) === date('Y-m-d'); })) ?>
                ...
                <div class="kpi-card">
                    ...
                    <div class="card-value">
                        <?= count($verificationApplications) ?>
                        <i class="fas fa-file-alt"></i>
                    </div>
                </div>
                
                <div class="kpi-card">
                    <div class="card-title">Available Badges</div>
                    <div class="card-value">
                        <?= count($allBadges) ?>
                            ...
                            <?php foreach ($verificationApplications as $app): ?>
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        <small style="color: #666;"><?= htmlspecialchars($app['email']) ?></small>
                                    </td>
                                    <td>
                                    ...
                                    <td><?= $app['reviewed_at'] ? date('M j, Y', strtotime($app['reviewed_at'])) : '-' ?></td>
                                    <td>
                                        <?php if ($app['status'] === 'pending'): ?>
                                            <div style="display: flex; gap: 5px;">
                                                <button class="btn btn-success btn-sm" onclick="approveVerification(<?= $app['id'] ?>)">
                                                    <i class="fas fa-check"></i> Approve
                                                </button>
                                                <button class="btn btn-danger btn-sm" onclick="rejectVerification(<?= $app['id'] ?>)">
            ...
            <div class="alert alert-info" style="margin-top: 30px;">
                ...
                <p>Review and approve tipster verification applications to maintain platform quality and trust.</p>
                <p><strong>Current Status:</strong> <?= count($verificationApplications) ?> total applications | <?= count(array_filter($verificationApplications, function($app) { return $app['status'] === 'pending'; })) ?> pending review</p>
            </div>
        </div>
    </div>
