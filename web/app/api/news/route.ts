...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (!res.ok) {
      console.error(`[NewsProxy] Backend error: ${res.status} ${res.statusText}`);
    ...
    console.error('[NewsProxy] Fetch failed:', err);
    return NextResponse.json([], { status: 500 }); // Return error status for debugging
  }
}
