  ...
  useEffect(() => {
    ...
    fetch(`${getApiUrl()}/admin/news/${id}`, { headers: { Authorization: `Bearer ${token}` } })
      .then(async (r) => {
        ...
        if (!r.ok) {
          setError(getApiErrorMessage(data, 'Article not found'));
          return null;
        }
        return data;
      })
  ...
  return (
    <div className="flex min-h-screen bg-[var(--bg)] w-full min-w-0 max-w-full overflow-x-hidden">
      ...
      <main className="admin-main-sibling section-ux-admin-main min-w-0">
        ...
        <h1 className="text-2xl sm:text-3xl font-bold text-gray-900 dark:text-white mb-6">Edit News Article</h1>
        <form onSubmit={submit} className="max-w-2xl space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            ...
            <Field label="Language" htmlFor="news-language">
              <select
                id="news-language"
                value={form.language}
