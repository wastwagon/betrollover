...
import { mixHomeTopPerforming } from '@/lib/home-top-performing';
...
function mapLeaderboardToTipsterCard(entry: Record<string, unknown>, index: number): TipsterCardData {
  ...
  const rankRaw = entry.rank ?? entry.leaderboard_rank;
  const rank =
    typeof rankRaw === 'number' && Number.isFinite(rankRaw) && rankRaw > 0 ? rankRaw : index + 1;
  return {
...
export function HomePopularTipsters({
  ...
  const t = useT();
  const seeded = mixHomeTopPerforming(initialLeaderboard).map((e, i) => mapLeaderboardToTipsterCard(e, i));
  ...
  useEffect(() => {
    const load = (showSpinner: boolean) => {
      ...
      fetch(getApiUrl() + '/leaderboard?period=all_time&limit=50', { cache: 'no-store' })
        ...
        .then((data) => {
          const entries = mixHomeTopPerforming((data.leaderboard || []) as Record<string, unknown>[]);
