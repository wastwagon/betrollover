...
FILES = [
    "AccaSure1X2-2026-08-14.json",
    "AccaSure1X2-2026-08-21.json",
    "AccaSure1X2-2026-08-28.json",
    "AccaSure1X2-2026-09-04.json",
    "AccaSure1X2-2026-09-11.json",
]
...
def main():
    ...
    cf(lambda t: slot_from_title(t.get("title") or "") == "Evening", "Evening only")
    cf(lambda t: slot_from_title(t.get("title") or "") == "Early", "Early only")
    # home-only vs away legs
    def has_away(t):
        return any((p.get("outcomeKey") or "").lower() in ("away", "2") or "away" in (p.get("prediction") or "").lower() for p in t.get("picks") or [])
    def home_only(t):
        keys = [(p.get("outcomeKey") or "").lower() for p in t.get("picks") or []]
        preds = [(p.get("prediction") or "").lower() for p in t.get("picks") or []]
        return all(k in ("home", "1") or "home" in pr for k, pr in zip(keys, preds)) if keys else False
    cf(lambda t: not has_away(t), "no away legs")
    cf(lambda t: has_away(t), "has away leg")
    cf(lambda t: not youth(t), "no youth")
    cf(lambda t: youth(t), "youth only")
    for lo, hi, lab in [(0, 1.55, "<1.55"), (1.55, 1.65, "1.55-1.65"), (1.65, 1.75, "1.65-1.75"), (1.75, 1.90, "1.75-1.90"), (1.90, 99, ">1.90")]:
