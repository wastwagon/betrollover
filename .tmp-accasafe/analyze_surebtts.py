...
FILES = [
    "AccaSureBTTS-2026-08-14.json",
    "AccaSureBTTS-2026-08-21.json",
    "AccaSureBTTS-2026-08-28.json",
    "AccaSureBTTS-2026-09-04.json",
    "AccaSureBTTS-2026-09-11.json",
]
...
def score_shape(p: dict) -> str:
    ...
    if h == 0 and a == 0:
        return "0-0"
    if h == 0:
        return "home-CS"  # away scored, home blank
    if a == 0:
        return "away-CS"  # home scored, away blank
    return f"both-scored?!"  # should not appear on lost BTTS Yes
...
def main():
    ...
    cf(lambda t: float(t["totalOdds"]) <= 1.70, "combined <= 1.70")
    cf(lambda t: float(t["totalOdds"]) <= 1.65, "combined <= 1.65")
    cf(lambda t: 1.55 <= float(t["totalOdds"]) <= 1.75, "1.55-1.75")
    cf(lambda t: float(t["totalOdds"]) < 1.75, "combined < 1.75")
    cf(lambda t: float(t["totalOdds"]) < 1.80, "combined < 1.80")
    cf(lambda t: slot_from_title(t.get("title") or "") != "Afternoon", "no Afternoon")
    cf(lambda t: slot_from_title(t.get("title") or "") == "Early" and float(t["totalOdds"]) < 1.75, "Early + combined < 1.75")
    cf(lambda t: slot_from_title(t.get("title") or "") in ("Early", "Evening"), "Early+Evening")
    # odds buckets finer for Sure
    for lo, hi, lab in [(0, 1.60, "<1.60"), (1.60, 1.70, "1.60-1.70"), (1.70, 1.80, "1.70-1.80"), (1.80, 1.95, "1.80-1.95"), (1.95, 99, ">1.95")]:
    ...
    print("\n=== ODDS BUCKETS ===")
    for lo, hi, lab in [(0, 1.55, "<1.55"), (1.55, 1.65, "1.55-1.65"), (1.65, 1.75, "1.65-1.75"), (1.75, 1.85, "1.75-1.85"), (1.85, 2.00, "1.85-2.00"), (2.00, 99, ">2.00")]:
