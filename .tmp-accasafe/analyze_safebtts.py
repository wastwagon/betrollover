...
FILES = [
    "AccaSafeBTTS-2026-08-14.json",
    "AccaSafeBTTS-2026-08-21.json",
    "AccaSafeBTTS-2026-08-28.json",
    "AccaSafeBTTS-2026-09-04.json",
    "AccaSafeBTTS-2026-09-11.json",
]
...
def main():
    ...
    cf(lambda t: float(t["totalOdds"]) <= 2.20, "combined <= 2.20")
    cf(lambda t: float(t["totalOdds"]) <= 2.40, "combined <= 2.40")
    cf(lambda t: float(t["totalOdds"]) <= 2.50, "combined <= 2.50")
    cf(lambda t: float(t["totalOdds"]) < 2.60, "combined < 2.60")
    ...
    cf(lambda t: slot_from_title(t.get("title") or "") == "Evening", "Evening only")
    cf(lambda t: slot_from_title(t.get("title") or "") in ("Early", "Afternoon"), "Early+Afternoon")
    cf(lambda t: not youth(t) and float(t["totalOdds"]) <= 2.40, "no youth + <=2.40")
    cf(lambda t: slot_from_title(t.get("title") or "") != "Midnight" and float(t["totalOdds"]) <= 2.40, "no Mid + <=2.40")
    for lo, hi, lab in [(0, 2.10, "<2.10"), (2.10, 2.30, "2.10-2.30"), (2.30, 2.50, "2.30-2.50"), (2.50, 2.80, "2.50-2.80"), (2.80, 3.20, "2.80-3.20"), (3.20, 99, ">3.20")]:
